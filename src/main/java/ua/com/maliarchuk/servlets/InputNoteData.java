...
import ua.com.maliarchuk.view.ViewNote;
 ...
 */
@WebServlet(name = "inputNoteData", urlPatterns = {"/inputNoteData"})
public class InputNoteData extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("UTF-8");
        response.setContentType("text/html;charset=UTF-8");

        String[] userIDs = request.getParameterValues("selectButton");
        String userID = userIDs[0];
        String[] comments = request.getParameterValues("comment");
        String textNote = comments[0];

        String textInfo = "";

        if (textNote.equals("")) {
            textInfo = "Нотатку не створено! Відсутній текст нотатки!";
        } else {
            new ViewNote().createNewNote(textNote, Integer.parseInt(userID));
            textInfo = "Нотатку успішно збережно!";
        }
