 ...
 */
@WebServlet(name = "inputUserData", urlPatterns = {"/inputUserData"})
public class InputUserData extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for(ModelUser user : DataBase.allUsers){
            if (user.getUserName().equals(userName)) isPresent=true;
        }
        if (isPresent) {
            textInfo = "Користувача не додано! Користувач з таким іменем вже існує!";
        } else {
            if (userName.equals("") | (password.equals(""))) {
                textInfo = "Користувача не додано! Не коректні дані!";
            }else{
                new ViewUser().createNewUser(userName, password);
                textInfo = "Користувача успішно додано!";
            }
        }
